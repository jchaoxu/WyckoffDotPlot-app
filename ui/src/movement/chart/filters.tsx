...
export const Filters: React.FC<FiltersProps> = ({ onSearch }) => {
  ...
  const HandleSubmit = useCallback(() => {
    ...
    onSearch(request);
  }, [decimalPlaces, endDate, frequency, minStep, onSearch, startDate, stockCode, stockPartyCode]);
  ...
  return <div className="filter-container">
    <Row className="section-row">
      <Col md={6}>
        <InputGroup>
          <SelectControl value={stockPartyCode} onChangeCallback={setStockPartyCode}>
    ...
    <Row className="section-row">
      <Col md={4}>
        <InputGroup>
          <InputGroup.Text>保留小数位</InputGroup.Text>
          <SelectControl value={decimalPlaces} onChangeCallback={setDecimalPlaces}>
